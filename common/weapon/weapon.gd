...
func fire_weapon() -> void:
	...
	# Play fire SFX (Odtwarza przypisany do danej sceny dźwięk)
	if firing_sfx:
		firing_sfx.play()
...
func _on_aim_mode_changed(aim_mode: bool) -> void:
	# Reagujemy na zmianę celowania tylko wtedy, gdy ta broń jest aktualnie wyciągnięta
	if not visible:
		return
	
	is_aimed = aim_mode
	
	# Resetujemy poprzedni proces płynnego przejścia rąk, jeśli gracz klika celowanie bardzo szybko
	if aim_tween:
		aim_tween.kill()
	
	if fps_arms_root:
		# Tworzymy płynną animację przejścia rąk do pozycji celowania (lub biodra)
		...
		# Wybieramy odpowiednią pozycję docelową z Marker3D
	...
	if aim_mode:
		...
		# Przywrócenie domyślnego FOV widoku (np. 75.0), kiedy gracz przestaje celować
		Global.active_camera_fov_changed.emit(75.0)
		if Global.debug_mode:
			print("AIM MODE: ", aim_mode, " - Weapon: ", name, " - FOV: 75.0")
